...
var watch = module.exports = (function () {
	function watch(file, frequency) {
		...
		this.logger = log.child({target: this.file})
			...
			fs.stat(this.file, function (err, stats) {
				...
				if (stats.isFile()) {
					try {
						...
						this.logger.info({event: 'watcher created'}, 'file found, created watcher')
					} catch (e) {
						if (e.code === 'ENOENT') {
							return callback()
						}
					...
					watcher.on('change', _.debounce(function (event, name) {
						this.logger.debug({event: event}, 'event', event, name)
						if (event !== 'rename') {
